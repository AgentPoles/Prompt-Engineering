...
def load_qwen_34b():
    """Load Qwen 34B with optimized settings for A100 80GB"""
    print("🚀 Loading Qwen 34B with 4-bit quantization...")
    ...
    model_name = "Qwen/Qwen2.5-34B-Instruct"
    ...
    try:
        ...
        print(f"✅ Qwen 34B loaded successfully!")
        if torch.cuda.is_available():
        ...
        try:
            ...
            print(f"✅ Qwen 34B loaded successfully (without flash attention)!")
            if torch.cuda.is_available():
                print(f"📊 GPU memory used: {torch.cuda.memory_allocated() / 1024**3:.1f}GB")
            return model, tokenizer
...
def create_sports_prompt(game_data):
    ...
    for team_name, team_data in teams.items():
        ...
        # Get top 4 players by points (34B can handle more detail)
...
def generate_report(model, tokenizer, prompt):
    """Generate sports report with Qwen 34B"""
    print("🏀 Generating sports report with Qwen 34B...")
    ...
    # Generation parameters optimized for Qwen 34B
    with torch.no_grad():
...
def main():
    ...
    print("=" * 80)
    print("🏀 QWEN 34B SPORTS JOURNALISM GENERATOR - A100 OPTIMIZED")
    print("=" * 80)
